...
CREATE TABLE "amp_abandoned_carts_native_connector" AS


SELECT  a.customer_uid,a.correlation_id,a.ip_address,case when a.country ='GB' THEN 'UK' ELSE a.country END as ip_country,a.storefront,a.cart_id,a.product_sku,a.product_id,a.product_category,a.preorder_status,a.event_time,a.item_number_in_cart,a.sum_quantity,MAX(a.event_time) OVER (PARTITION BY cart_id) as cart_last_touched_time,a.event_type,a.logged_in,
   case
  ...
  ,b.authorized_to_sell                        
   from src_amplitude_pokemoncenter.abandoned_carts a  INNER JOIN tpci_stg.sflake_dim_product b 
ON a.product_sku = b.sku
AND a.storefront =b.storefront_2_char_format
AND a.event_type = 'Added to Cart'
ORDER BY a.product_sku,a.product_id